...
begin
 ...
 Begin
  ...
  Begin
    Close;Clear;
    add('Select GroupRekID from CFMsGroupRek Where GroupRekId='''+quMainGroupRekId.Value+''' ');
    Open;
    If Not IsEmpty then
    Begin
...
begin
  ...
  with TfmSearch.Create(Self) do
      begin
       try
         Title := 'Valuta';
         SQLString := ' Select GroupRekID as Kode_Group_Rek, GroupRekName as Nama_Group_Rek '
                     +' FROM CFMsGroupRek'
                     +' ORDER BY GroupRekID';
         if ShowModal = MrOk then
         begin
           qumain.Locate('GroupRekID',Res[0],[]);
         end;
       finally
         free;
       end;
...
begin
  ...
  begin
    ...
    if not IsEmpty then
    Begin
      MsgInfo('Kode Group ini sudah di pakai di Master Rekening');
      Abort;
    End;
  end;
end;
