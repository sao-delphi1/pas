...
uses ConMain, UnitGeneral, Search, StdLv1;
...
begin
  ...
  begin
    ShowMessage('Kode Rekening tidak boleh kosong');
    quMainRekeningID.FocusControl;
    Abort;
  end;
  if TRIM(quMainLRekeningName.AsString) = '' then
  begin
    ShowMessage('Kode Rekening belum terdaftar dalam Master Rekening');
    quMainRekeningID.FocusControl;
    Abort;
  end;
  ...
  begin
    Close;Clear;
    Add('SELECT BankName FROM CFMsBank WHERE RekeningID=:RekeningID AND BankID<>:BankID ');
    Parameters.ParamByName('RekeningID').Value := quMainRekeningID.AsString;
    Parameters.ParamByName('BankID').Value := quMainBankId.AsString;
    Open;
    if not IsEmpty then
    begin
      ShowMessage('Kode Rekening sudah terdaftar atas Bank ['+quAct.FieldByName('BankName').AsString+']');
      quMainRekeningID.FocusControl;
      Abort;
    end;
  end;
...
begin
  ...
  with TfmSearch.Create(Self) do
      begin
       try
         Title := 'Master Bank';
         SQLString := 'SELECT BankName as Nama_Bank,'
                     +'CASE WHEN FgActive=''Y'' THEN ''Active'' ELSE ''Non Active'' END as Status,BankID as Kode_Bank '
                     +'FROM CFMsBank ORDER BY BankID';
         if ShowModal = MrOk then
         begin
            qumain.Locate('BankId',Res[2],[]);
         end;
       finally
         free;
       end;
...
begin
  ...
  begin
    Close;Clear;
    Add('SELECT RekeningName FROM CFMsRekening WHERE RekeningID=:RekeningID');
    Parameters.ParamByName('RekeningID').Value := quMainRekeningID.AsString;
    Open;
  end;
