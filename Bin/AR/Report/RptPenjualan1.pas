...
begin
  ...
  sQuery := 'SELECT L.*,M.SalesName,CASE WHEN L.GP > ISNULL(M.TOmzet,0) THEN L.Margin*M.Kom1*0.01 ELSE L.Margin*M.Kom2*0.01 END as HKomisi FROM('
           +'SELECT K.*,ISNULL(Jual-Modal,0) as Margin,ISNULL(PPNOut-PPNIn,0) as PPNMargin,'
           ...
           +'INNER JOIN CFTrKKBBHd B ON A.VoucherID=B.VoucherID AND B.FlagKKBB IN (''ARK'',''ARB'',''ARC'') '
           +'WHERE A.RekeningID='''+sDRPj+''' AND A.Note=K.SaleID ORDER BY CONVERT(VARCHAR(8),B.Transdate,112) DESC) as Lama FROM ('
           ...
           +'ISNULL(ROUND((A.Qty*A.Modal/(1+(B.PPNFee*0.01))*(B.PPNFee*0.01)),2),0) as PPNIn,ISNULL(A.Qty*A.Komisi,0) as Komisi,ISNULL(B.TTLPj,0) as TotalInv FROM ARTrPenjualandt A '
           +'Inner join ARTrPenjualanHd B on A.SaleID=B.SaleID INNER JOIN ARMsCustomer C on B.CustID=C.CustID '
           +'INNER JOIN INMsItem D on A.ItemID=D.ItemID) as K) as L INNER JOIN ARMsSales M ON L.SalesID=M.SalesID '
...
{  sQuery := ' SELECT K.SaleID,K.SalesID,K.SalesName,K.Tgl,K.CustName,K.ItemName,K.FgTax,K.TransDate,K.Jual,'
           +' K.PPNOut,K.Modal,K.PPNIn,Jual-Modal as Margin,PPNOut-PPNIn as PPNMargin FROM ( '
           +' SELECT A.SaleID,B.SalesID,E.SalesName,CONVERT(VARCHAR(10),B.TransDate,103) as Tgl,C.CustName,D.ItemName,B.FgTax,B.TransDate,'
           +' ISNULL(ROUND((CASE WHEN B.FgTax=''Y'' THEN (A.Qty*A.Price)*1.1 ELSE (A.Qty*A.Price) END),2),0) as Jual,'
           +' ISNULL(ROUND((CASE WHEN B.FgTax=''Y'' THEN (A.Qty*A.Price)*0.1 ELSE 0 END),2),0) as PPNOut,'
           +' ISNULL(ROUND(A.Qty*A.Modal,2),0) as Modal,ISNULL(ROUND((A.Qty*A.Modal/1.1*0.1),2),0) as PPNIn,'
           +' ISNULL(A.Komisi,0) as Komisi FROM ARTrPenjualandt A '
...
procedure TfmRptPenjualan.quActCalcFields(DataSet: TDataSet);
begin
  inherited;
  with quAct1,SQL do
  begin
    Close;CLear;
    Add('select SOID FROM ARtrpenjualanhd where saleid='''+quActSaleID.AsString+''' ');
    Open;
  end;
  quActSOID.ASString := quAct1.FieldByName('SOID').ASString;
end;
