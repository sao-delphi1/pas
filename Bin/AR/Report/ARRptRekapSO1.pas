...
begin
  ...
  sQuery := 'SELECT A.POID,CONVERT(VARCHAR(10),A.Transdate,103) as TglSO,D.CustName,E.SalesName,'
           +'X.SaleID,CONVERT(VARCHAR(10),X.TglInvoice,103) as TglInvoice,Z.GBUID,CONVERT(VARCHAR(10),Z.TglPO,103) as TglPO,Z.SuppName,Z.KonsinyasiID,'
           +'CONVERT(VARCHAR(10),Z.TglTerima,103) as TglTerima,Z.PurchaseID,CONVERT(VARCHAR(10),Z.TglBeli,103) as TglBeli,'
           ...
           +'LEFT JOIN ('
           +'  SELECT A.SaleID,A.TransDate as TglInvoice,B.ItemID,A.SOID,B.Price,A.CustID '
           ...
           +'LEFT JOIN ('
           +'  SELECT A.GBUID,A.Transdate as TglPO,C.SuppName,B.ItemID,B.Price,A.SOID,Y.KonsinyasiID,Y.TglTerima,Y.PurchaseID,Y.TglBeli FROM ARTrPenawaranHd A '
           +'  INNER JOIN ARTrPenawaranDt B ON A.GBUID=B.GBUID '
           +'  INNER JOIN APMsSupplier C ON A.CustID=C.SuppID '
           +'  LEFT JOIN ('
           +'    SELECT A.KonsinyasiID,A.TransDate as TglTerima,A.SuppID,B.ItemID,A.POID,X.PurchaseID,X.TransDate as TglBeli FROM APTrKonsinyasiHd A '
           +'    INNER JOIN APTrKonsinyasiDt B ON A.KonsinyasiID=B.KonsinyasiID '
           +'    LEFT JOIN ('
           +'      SELECT A.PurchaseID,A.TransDate,A.SuppID,B.ItemID,A.KonsinyasiID FROM APTrPurchaseHd A '
