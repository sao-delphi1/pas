  ...
  begin
    with TfmARQRRptPenjPerCust.Create(Self) do
     try
       ...
       begin
         Close;Clear;
         add('SELECT DISTINCT K.SaleId,K.CustId,K.Tgl,K.Tgl2,K.CurrId,K.TTLPj,K.PPN,K.SubTotal,K.TaxID,K.Rate '
            +'FROM ( '
       ...
       begin
         Close;Clear;
         add(' SELECT Distinct K.SaleId,K.ItemId,K.ItemName,K.Qty,K.Price,K.Total '
            +' FROM ( '
            +' SELECT distinct A.SaleId,A.ItemId,B.ItemName,A.Qty,(A.Price+A.Komisi) As Price,(A.Qty*(A.Price+A.Komisi)) as Total'
