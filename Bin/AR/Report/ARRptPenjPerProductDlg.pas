...
begin
  ...
  with TfmARQRRptPenjPerProduct.Create(Self) do
     try
       ...
       begin
         Close;Clear;
         add(' Select Distinct K.ItemId,K.saleid,K.Tgl,K.Tgl2,K.QTY,K.CustName,K.CurrId,K.Price,K.Total,K.POID '
            +' FROM ( '
            +' SELECT distinct A.ItemId,C.SaleId,Convert(varchar(10),TransDate,103) as Tgl,Convert(varchar(8),TransDate,112) as tgl2,'
            +' A.QTY,B.CustName,C.CurrId,(A.Price+A.Komisi) as Price,((A.Price+A.Komisi)*A.Qty) as Total,C.POID '
