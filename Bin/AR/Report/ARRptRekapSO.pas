...
type
  TfmARRptRekapSODlg = class(TfmRptDlg)
    ...
    quActATotalModal: TCurrencyField;
    quActATotalJual: TCurrencyField;
    quActAMargin: TCurrencyField;
    quActATotalMargin: TCurrencyField;
...
begin
  ...
  sQuery := 'SELECT A.POID,CONVERT(VARCHAR(10),A.Transdate,103) as TglSO,D.CustName,E.SalesName,'
           ...
           +'LEFT JOIN ('
           +'  SELECT A.GBUID,A.Transdate as TglPO,C.SuppName,B.ItemID,Y.Price,A.SOID,Y.KonsinyasiID,Y.TglTerima,Y.PurchaseID,Y.FPSID,Y.TglBeli FROM ARTrPenawaranHd A '
           +'  INNER JOIN ARTrPenawaranDt B ON A.GBUID=B.GBUID '
           +'  INNER JOIN APMsSupplier C ON A.CustID=C.SuppID '
           +'  LEFT JOIN ('
           +'    SELECT A.KonsinyasiID,A.TransDate as TglTerima,A.SuppID,B.ItemID,A.POID,X.PurchaseID,X.FPSID,X.TransDate as TglBeli,X.Price FROM APTrKonsinyasiHd A '
           +'    INNER JOIN APTrKonsinyasiDt B ON A.KonsinyasiID=B.KonsinyasiID '
           +'    LEFT JOIN ('
           +'      SELECT A.PurchaseID,A.TransDate,A.SuppID,B.ItemID,A.KonsinyasiID,A.FPSID,B.Price FROM APTrPurchaseHd A '
...
procedure TfmARRptRekapSODlg.quActCalcFields(DataSet: TDataSet);
begin
  inherited;
  quActAMargin.AsCurrency := quActPrice.AsCurrency-quActModal.AsCurrency;
  quActATotalJual.AsCurrency := quActQty.AsCurrency*quActPrice.AsCurrency;
  quActATotalModal.AsCurrency := quActQty.AsCurrency*quActModal.AsCurrency;
  quActATotalMargin.AsCurrency := quActQty.AsCurrency*(quActPrice.AsCurrency-quActModal.AsCurrency);
end;
