...
begin
  ...
  sQuery := 'SELECT L.*,M.SalesName,CASE WHEN L.GP > ISNULL(M.TOmzet,0) THEN L.Margin*M.Kom1*0.01 ELSE L.Margin*M.Kom2*0.01 END as HKomisi FROM( '
           ...
           +'ISNULL((SELECT SUM(Y.Price) FROM ARTrPenjualanSN X INNER JOIN APTrPurchaseDtSN Y on Y.SNID=X.SNID WHERE A.SaleID=X.SaleID AND X.ItemID=A.ItemID AND Y.ItemID=A.ItemID),0) as ModalInvBeli, '
           +'ISNULL((SELECT SUM(X.Price-Y.Price) FROM ARTrPenjualanSN X INNER JOIN APTrPurchaseDtSN Y on Y.SNID=X.SNID WHERE A.SaleID=X.SaleID AND X.ItemID=A.ItemID AND Y.ItemID=A.ItemID),0) as MarginInvBeli '
