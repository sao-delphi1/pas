...
type
  TfmARSN = class(TfmStdLv0)
    ...
    dxButton2: TdxButton;
    quAct1: TADOQuery;
    quSOSOID: TStringField;
    quSOGBUID: TStringField;
    dbgSOColumn7: TdxDBGridColumn;
    dbgSOColumn8: TdxDBGridColumn;
    dbgSOColumn9: TdxDBGridColumn;
    Label1: TLabel;
    Edit1: TEdit;
    Label2: TLabel;
    Edit2: TEdit;
...
begin
  ...
  begin
    ...
    Add(' SELECT K.SNID as Serial_Number,K.KonsinyasiID as Nota_Pembelian,'
       ...
       +' CASE WHEN K.CurrID=''IDR'' THEN K.Price ELSE K.Price*K.Rate END as Modal,K.FgSN,K.SOID,K.GBUID FROM ( '
       +' SELECT A.SNID, C.KonsinyasiID, C.Transdate, C.SuppID, D.SuppName,'
       +' B.ItemID, F.ItemName, A.FgJual,B.Price,B.CurrID,C.Rate,A.FgSN,G.SOID,G.GBUID '
       ...
       +' INNER JOIN ARTRPenawaranHd G on C.POID=G.GBUID '
       ...
       +' AND K.SOID LIKE ''%'+Edit1.Text+'%'' AND K.GBUID LIKE ''%'+Edit2.Text+'%'' '
       +' ORDER BY K.SNID ');
    Open;
  end;
  with quDO,SQL do
...
begin
  ...
  SetReadOnly(dbgSOColumn7,TRUE);
  SetReadOnly(dbgSOColumn8,TRUE);
  SetReadOnly(dbgSOColumn9,TRUE);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        Close;Clear;
        Add('INSERT ARTrPenjualanSn (SNID,SaleID,ItemID,Price,WarehouseID,UpdDate,UpdUser,Modal,PurchaseID,FgSN)');
        Add('SELECT TOP '+IntToStr(Jumlah)+' K.SNID,'''+SaleID+''',K.ItemID,'''+Price+''','''+Gudang+''',GETDATE(),'''+dmMain.UserId+''',K.Modal,K.KonsinyasiID,K.FgSN '
           +'FROM ('
           +'SELECT A.SNID,A.KonsinyasiID,B.ItemID,A.FgJual,ISNULL(B.Price,0) as Modal,A.FgSN,G.SOID,G.GBUID '
           +'FROM APTrKonsinyasiDtSn A '
           +'INNER JOIN APTrKonsinyasiDt B ON A.KonsinyasiID=B.KonsinyasiID AND A.ItemID=B.ItemID '
           +'INNER JOIN APTrKonsinyasiHd C ON B.KonsinyasiID=C.KonsinyasiID '
           +'INNER JOIN ARTRPenawaranHd G on C.POID=G.GBUID '
...
procedure TfmARSN.Button1Click(Sender: TObject);
begin
  inherited;
  RefreshData;
end;
