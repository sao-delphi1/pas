...
uses Search, UnitGeneral, ConMain, MyUnit,
  ...
  ARTrPiutangIDR, ARSN, StdLv3, StdLv0;
...
Begin
  ...
  begin
    ...
    begin
      Close;Clear;
      Add('SELECT K.SubTotal,K.DP*0.1 as Pajak,K.DP*1.1 as GT,K.modal FROM ('
         +'SELECT ISNULL(SUM(Qty*Price),0) as SubTotal,ISNULL(B.DP,0) as DP,ISNULL(SUM(Qty*Price),0)  as Modal '
         +'FROM ARTrPerformaDt A INNER JOIN ARTrPenjualanHd B ON A.PerformaID=B.KonTransBrgID '
         +'WHERE B.SaleID='''+quMainSaleID.AsString+''' GROUP BY B.DP) as K');
      Open;
    end;
  end else
  begin
    ...
    begin
      Close;Clear;
      Add('SELECT K.SubTotal,(K.SubTotal-K.DP)*0.1 as Pajak,(K.SubTotal-K.DP)+(K.SubTotal-K.DP)*0.1 as GT,K.modal FROM ('
         ...
         +'WHERE A.SaleID='''+quMainSaleID.AsString+''' GROUP BY B.DP) as K');
      Open;
    end;
  end;
End;
...
begin
  ...
  begin
    ...
    begin
      Close;Clear;
      Add('SELECT SaleID,TaxID FROM ARTrPenjualanHd WHERE TaxID='''+quMainTaxID.AsString+''' ');
      Open;
      if not IsEmpty then
      begin
...
begin
  inherited;
  SubTotal := (quDetilPrice.AsCurrency * quDetilQty.AsCurrency);
  ...
  with TfmSearch.Create(Self) do
    try
       ...
       SQLString := 'SELECT A.SaleID as Nota_Penjualan,A.TaxID as Faktur_Pajak, '
                   +'Convert(varchar(10),TransDate,103) as Tanggal,A.POID as No_PO,CustName as Pelanggan,'
...
begin
  ...
  Begin
     ...
     try
       ...
       Begin
         Close;Clear;
         add(' SELECT A.FgTax,B.SalesName as Sales,''INV-''+A.SaleId as SaleID,A.TaxID,A.CurrId,A.Discount,isnull(DP,0) as DP'
       ...
       begin
         ...
         Begin
           Close;Clear;
           add('select A.ItemID,A.ItemName,ISNULL(A.Qty,0) as Qty,ISNULL(A.Price,0) as Price,'
              +'ISNULL(A.Qty*A.Price,0) as total,B.UOMID,B.PartNo,B.Note '
       ...
       begin
         ...
         Begin
           Close;Clear;
           add('SELECT Note as ItemName,UOMId,ISNULL(Qty,0) as Qty,ISNULL(Price,0) as Price,Note2 as Note '
              +'FROM ARTrPenjualanDt WHERE SaleId='''+quMainSaleID.AsString+'''  ORDER BY Urut');
           Open;
         End;
       end;
  ...
  Begin
    ...
    begin
     ...
     try
       ...
       Begin
         Close;Clear;
         add(' SELECT A.FgTax,B.SalesName as Sales,''DO-''+A.SaleId as SaleID,A.TaxID,A.CurrId,A.Discount,isnull(DP,0) as DP'
            +' ,isnull((TtlPj-DP),0) as GT,isnull(TTLPj,0) as Total,Convert(varchar(12),Transdate,106) as Tgl,A.CustId,'
            +' CASE WHEN ISNULL(A.Actor,'''')='''' THEN C.CustName ELSE C.CustName+'' (''+A.Actor+'')'' END as Custname,'
            +' C.Address as Address,C.City,A.Alamat,A.POID,'
            +' CASE WHEN ISNULL(JatuhTempo,0) > 0 THEN CONVERT(VARCHAR(12),DATEADD(DAY,JatuhTempo,Transdate),106)+'' (''+CAST(JatuhTempo as VARCHAR(3))+'' Hari)'' '
       ...
       Begin
         Close;Clear;
         add(' Select A.Urut,A.ItemId,A.Note as ItemName,A.UOMId,A.Qty,A.Price,A.Note2 as Note FROM ARTrPenjualanDt A'
...
begin
  ...
  begin
    Close;Clear;
    Add('SELECT A.CustID,A.SalesID,ISNULL(A.Administrasi,0) as Ongkir FROM ARTrPurchaseOrderHd A '
       +'WHERE A.POID='''+quMainSOID.AsString+''' ');
    Open;
  end;
...
begin
  ...
  begin
    Close;Clear;
    Add('SELECT ISNULL(Term,0) as Term FROM ARMsCustomer WHERE CustID='''+quMainCustID.AsString+''' ');
    Open;
  end;
...
begin
  ...
  try
     Title := 'Cari Nomor FP';
     SQLString := 'SELECT TaxID FROM ARMsKdPajak WHERE TaxID NOT IN (SELECT ISNULL(TaxID,''-'') FROM ARTrPenjualanHd) ORDER BY TaxID';
     if ShowModal = MrOK then
     begin
        if quMain.State = dsBrowse then quMain.Edit;
