...
type
  TfmINMSItem = class(TfmStdLv4)
    ...
    quMainKodeJenisBarang: TStringField;
    quMainKodeSatuan: TStringField;
    quMainFgBarang: TStringField;
...
begin
  ...
  quMainFgBarang.AsString := 'A';
  quMainKodeJenisBarang.AsString := '000000';
  quMainProductID.FocusControl;
end;
...
begin
  ...
  with TfmSearch.Create(Self) do
      begin
       try
         Title := 'Cari Satuan';
         SQLString := ' SELECT UOMID as Nama_Satuan,KodeSatuan as Kode_Satuan '
                     +' FROM INMsUOM'
                     +' ORDER BY UOMID';
         if ShowModal = MrOk then
         begin
           if quMain.State = dsBrowse then quMain.edit;
              quMainUOMID.AsString:= res[0];
              quMainKodeSatuan.AsString := res[1];
         end;
       finally
         free;
       end;
