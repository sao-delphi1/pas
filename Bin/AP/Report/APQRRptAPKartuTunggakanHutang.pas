...
begin
  ...
  Begin
    ...
    add('SELECT DATEADD(DAY, ISNULL(K.JatuhTempo,0),K.Transdate) as jth,K.SuppID,CONVERT(VARCHAR(10),K.TransDate,103) as Tanggal,K.PurchaseID as IP,'
       +'ISNULL(K.Kredit,0) as Kredit,(SELECT ISNULL(SUM(L.Amount),0) FROM CFTrKKBBDt L INNER JOIN CFTrKKBBHd Q ON L.VoucherID=Q.VoucherID '
       +'WHERE L.Note = K.PurchaseID AND CONVERT(VARCHAR(8),Q.Transdate,112) <= '''+FormatDateTime('yyyyMMdd',tglDari)+''') as Debit '
       +'FROM ( '
       ...
       +'CONVERT(VARCHAR(8),G.Transdate,112) <= '''+FormatDateTime('yyyyMMdd',tglDari)+''' AND G.SuppID=A.SuppID),0) as Kredit '
       +'FROM APTrPurchaseHd A '
       +') as K '
       +'WHERE K.SuppID = '''+qu001.Fieldbyname('SuppId').AsString+''' AND K.CurrID='''+qu002.FieldByname('CurrId').AsString+''' '
       +'AND ISNULL(K.Kredit-(SELECT ISNULL(SUM(L.Amount),0) FROM CFTrKKBBDt L INNER JOIN CFTrKKBBHd Q ON L.VoucherID=Q.VoucherID '
       +'WHERE L.Note = K.PurchaseID AND CONVERT(VARCHAR(8),Q.Transdate,112) <= '''+FormatDateTime('yyyyMMdd',tglDari)+'''),0) > 0 '
