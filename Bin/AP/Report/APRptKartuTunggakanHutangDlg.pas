  ...
  try
    ...
    Begin
       ...
       add('SELECT DISTINCT K.SuppID,L.SuppName,(K.SuppID+''-''+L.SuppName) as Supplier FROM ('
          ...
          +'WHERE M.Note=A.PurchaseID AND CONVERT(VARCHAR(8),N.TransDate,112) <= '''+FormatDateTime('yyyyMMdd',dt1.Date)+'''),0) as Bayar,'
    ...
    Begin
       ...
       add('SELECT DISTINCT K.SuppID,K.CurrID,L.CurrName,(K.CurrID+''-''+L.CurrName) as Valuta FROM ('
          ...
          +'WHERE N.Note=B.PurchaseID AND CONVERT(VARCHAR(8),M.TransDate,112) <= '''+FormatDateTime('yyyyMMdd',dt1.Date)+'''),0) as Bayar '
