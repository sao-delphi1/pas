  ...
  begin
    ...
    begin
      ...
      Add('SELECT A.PurchaseID,B.SuppName,CONVERT(VARCHAR(10),A.TransDate,103) as Tgl,A.JatuhTempo as Term, '
         ...
         +'A.UpdUser,A.UpdDate,A.FgOto as Jenis, '
         +'(CASE WHEN A.FgOto=''Y'' THEN ''Approved'' ELSE ''Pending'' END) as StatusOto,0.00 as Margin '
  ...
  begin
    ...
    begin
      ...
      Add('SELECT A.POID as PurchaseID,B.CustName as SuppName,CONVERT(VARCHAR(10),A.Transdate,103) as Tgl,0 as Term,'
         +'C.SalesName as JthTempo,ISNULL(A.TTLSO,0) as TTLPb,A.UpdUser,A.UpdDate,A.Jenis,'
         +'CASE WHEN A.Jenis=''D'' THEN ''Overdue'' '
         +'     WHEN A.Jenis=''L'' THEN ''Overlimit'' '
         +'     WHEN A.Jenis=''R'' THEN ''Jual Rugi'' '
         +'     WHEN A.Jenis=''DL'' THEN ''Overdue & Overlimit'' '
         +'     WHEN A.Jenis=''DR'' THEN ''Jual Rugi $ Overdue'' '
         +'     WHEN A.Jenis=''LR'' THEN ''Jual Rugi $ Overlimit'' '
         +'     WHEN A.Jenis=''DLR'' THEN ''Jual Rugi, Overdue & Overlimit'' END as StatusOto,A.Margin '
         +'FROM ARTrPurchaseOrderHd A INNER JOIN ARMsCustomer B ON A.CustID=B.CustID '
         +'INNER JOIN ARMsSales C ON A.SalesID=C.SalesID WHERE A.Jenis NOT IN (''Y'',''T'',''X'') '
...
begin
  ...
  begin
    ...
    begin
      Close;Clear;
      Add('select ISNULL(FgOtoSO,''0'') as OtoLevel,ISNULL(OtoMargin,2) as OtoMargin FROM SysMsUser Where UserID='''+dmMain.UserId+''' ');
      Open;
    end;
    OtoLevel := quAct.FieldByName('OtoLevel').AsString;
    OtoMargin := quAct.FieldByName('OtoMargin').AsCurrency;

    if (OtoLevel='0') then
    begin
      ShowMessage('Anda Tidak Punya Hak Otorisasi!');
      Abort;
    end else
    if (OtoLevel='1') or (OtoLevel='2') then
    begin
      if (quMainJenis.AsString='DR') or (quMainJenis.AsString='LR') or (quMainJenis.AsString='R') or (quMainJenis.AsString='DLR') then
      begin
        ShowMessage('Anda Tidak Punya Hak Otorisasi untuk Jual Rugi!');
        Abort;
      end;

      if (quMainMargin.AsCurrency <= OtoMargin) then
      begin
        ShowMessage('Anda Hanya Bisa Otorisasi Margin diatas '+CurrToStr(OtoMargin)+'% !');
        Abort;
      end;
    end;
  end;
