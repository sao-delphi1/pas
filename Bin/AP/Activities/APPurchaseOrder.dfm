inherited fmAPPurchaseOrder: TfmAPPurchaseOrder
  ...
  inherited quMain: TADOQuery
    ...
    object quMainFPSID: TStringField
      FieldName = 'FPSID'
      Size = 30
    end
    ...
    object quMainKonsinyasiID: TStringField
      ...
      OnChange = quMainKonsinyasiIDChange
      Size = 50
    end
    ...
    object quMainLRekBeli: TStringField
      FieldKind = fkCalculated
      FieldName = 'LRekBeli'
      Size = 100
      Calculated = True
    ...
    object quMainLRekUtang: TStringField
      FieldKind = fkCalculated
      FieldName = 'LRekUtang'
      Size = 100
      Calculated = True
    end
    object quMainRatePPN: TBCDField
      FieldName = 'RatePPN'
      DisplayFormat = '#,0.00'
      EditFormat = '#,0.00'
      Precision = 18
    ...
    object quMainLPOID: TStringField
      FieldKind = fkCalculated
      FieldName = 'LPOID'
      Size = 100
      Calculated = True
    end
    object quMainTermAsli: TStringField
      FieldKind = fkCalculated
      FieldName = 'TermAsli'
      Size = 100
      Calculated = True
    end
    object quMainLSOID: TStringField
      FieldKind = fkCalculated
      FieldName = 'LSOID'
      Size = 300
      Calculated = True
  ...
  object quTotal: TADOQuery
    ...
    SQL.Strings = (
        ...
        'SELECT ROUND(K.SubTotal,2) as SubTotal,ROUND(CASE WHEN FgTax='#39'Y'#39 +
        ' THEN K.SubTotal*K.Rate*0.01 ELSE 0 END,2) as PPN,'
      
        'ROUND(ROUND(K.SubTotal,2)+ROUND((CASE WHEN FgTax='#39'Y'#39' THEN K.SubT' +
        'otal*K.Rate*0.01 ELSE 0 END),2),2) as GT FROM ('
        ...
        'D=B.PurchaseID'
      'GROUP BY A.PurchaseID,A.SuppID,B.FgTax,B.RatePPN) as K '
      'WHERE K.PurchaseId=:PurchaseID AND K.SuppId=:SuppId')
    Left = 768
    Top = 372
    object quTotalSubTotal: TFloatField
      FieldName = 'SubTotal'
      DisplayFormat = '#,0.00'
    end
    object quTotalPPN: TFloatField
      FieldName = 'PPN'
      ReadOnly = True
      DisplayFormat = '#,0.00'
    end
    object quTotalGT: TFloatField
      FieldName = 'GT'
      ReadOnly = True
      DisplayFormat = '#,0.00'
    end
